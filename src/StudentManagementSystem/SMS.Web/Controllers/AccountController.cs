...
namespace SMS.Web.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Register(RegisterModel model)
        {
            ...
            var isReCaptchaValid = _reCaptchaService.IsReCaptchaValid(model.ReCaptchaToken);

            if (!isReCaptchaValid.Result)
            {
                ModelState.AddModelError(string.Empty, "Prove you are not a bot.");
                return View();
            }
            ...
            if (ModelState.IsValid)
            {
                ...
                await _userManager.AddClaimAsync(user, new Claim("CompanyAdmin", "true"));
        ...
        {
            var companyModel = _scope.Resolve<CreateCompanyModel>();
            model.Resolve(_scope);
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    //companyModel.AdminUserId = Guid.Parse(this.User.FindFirstValue(ClaimTypes.NameIdentifier));
                    //companyModel.Name = model.Name;
                    //await companyModel.CreateCompanyAsync();
                }
                catch (Exception ex)
                {
                    ModelState.AddModelError("", "Failed to Create Company");
                    _logger.LogError(ex, "Company create failed");
                }
            }
